...
if __name__ == "__main__":
    ...
    phone = sys.argv[1]
    img_path = sys.argv[2]
    msg_caption = sys.argv[3]

    print(f"--- Starting Image Send GUI Script (Copy-Paste Img+Caption + Enter - NO CLICKS Method) ---")
    print(f"To: {phone}")
    print(f"Image: {img_path}")
    print(f"Caption: {msg_caption}")

    if not os.path.exists(img_path):
        print(f"Error: Image path does not exist: {img_path}")
        sys.exit(1)

    # Chama a função principal sem cliques
    if send_whatsapp_image_no_clicks(phone, img_path, msg_caption):
        print("--- Script finished: Success (GUI automation reported success) ---")
        sys.exit(0)
    else:
